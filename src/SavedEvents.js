...
const SavedEvents = () => {
  ...
  useEffect(() => {
    ...
    const savedActivitiesData = JSON.parse(localStorage.getItem("savedActivities")) || [];
        ...
        <ul>
          {savedMuseums.map((museum) => (
            <li key={museum._id}>
              ...
              <p><strong>Foreigner Price:</strong> ${museum.foreignerTicketPrice}</p>
              <p><strong>Student Price:</strong> ${museum.studentTicketPrice}</p>
              <p><strong>Native Price:</strong> ${museum.nativeTicketPrice}</p>
            </li>
          ))}
        </ul>
      )}
